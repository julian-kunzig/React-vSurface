  ...
  _createClass(Slider, [{
    ...
    value: function render() {
      ...
      var _props = this.props,
          ...
          innerTrackOffset = _props.innerTrackOffset || 0,
          handleTrackHeight = _props.handleTrackHeight || 0,
          handleTrackTop = _props.handleTrackTop || 0;
    ...
    var handleHeight = 0; //= _this2.handle.clientHeight || 0 ;                    
    //var sliderHeight = 0;
//    var innerTrackOffset = 8; // offset the usable range (to take account of the inner track being less tall than the full track
    if (_this2.handle){
        handleHeight= _this2.handle.clientHeight || 0;
        //sliderHeight = _this2.slider.clientHeight-2;                // the total height of the slider (i.e. 350px) dunno why i need the 2px offset!
        
    }
    //var trackRange = coords.handle + coords.fill;
    var trackRange = coords.handle + coords.fill;  // this is the trackHeight MINUS the handle height.
    
    //var thumbRange = trackRange + handleHeight - innerTrackOffset;
    //var thumbRange = sliderHeight - innerTrackOffset
    var thumbRange = handleTrackHeight;
    //var adjPos =  ((coords.handle/trackRange)*thumbRange) + (innerTrackOffset/2) - (handleHeight/2)
    var adjPos =  ((coords.handle/trackRange)*thumbRange) + (handleTrackTop) - (handleHeight/2)
    //var adjFill = thumbRange-((coords.handle/trackRange)*thumbRange)  - (handleHeight/2);
    var adjFill = thumbRange-((coords.handle/trackRange)*thumbRange)  - (handleHeight/2);
    // map from the native slider calc to our thumbrange and then add half the offset (cos its spread top and bottom) and then take into account the handleHeight


        if (adjFill<(0)){adjFill=0};
        var fillStyle = _defineProperty({}, dimension, adjFill + 'px');
        fillStyle.bottom = (handleTrackTop-1) + "px";

        //fillStyle += ' bottom: 6px;'
        var handleStyle = _defineProperty({}, direction, (adjPos) + 'px');
        //var handleStyle = _defineProperty({}, direction, (coords.handle) + 'px');
     //var fillStyle = _defineProperty({}, dimension, coords.fill + 'px');
      //var handleStyle = _defineProperty({}, direction, (coords.handle) + 'px');
      //console.log(handleHeight, trackRange, thumbRange, coords.handle, adjPos, adjFill);
     //=============
      ...
      if (labelKeys.length > 0) {
        ...
        try {
          for (var _iterator = labelKeys[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            var adjLabelPos =  ((labelCoords.label/trackRange)*thumbRange) + (innerTrackOffset/2) //sk edit
            //adjLabelPos = 100-key;
            adjLabelPos = 100 - (key/max*100)
            //var labelStyle = _defineProperty({}, direction, labelCoords.label + 'px');
            //var labelStyle = _defineProperty({}, direction, adjLabelPos + 'px');
            var labelStyle = _defineProperty({}, direction, adjLabelPos + '%');
            labelStyle['--line-height'] = '1px';
            if (this.props.labels[key].length>0){
                labelStyle['--line-height'] = '2px';
            }
            labelItems.push(_react2.default.createElement(
              'lix',
              {
                key: key,
                className: (0, _classnames2.default)('rangeslider__label-item2'),
                'data-value': key,
      ...
      //console.log(thumbRange, adjFill)
      ...
      return _react2.default.createElement(
        ...
        _react2.default.createElement('div', { className: 'rangeslider__track'}),
        _react2.default.createElement('div', { className: 'tick-container tick-containerL' },  _react2.default.createElement('div', { className: 'ticks ticksL' })),
        // _react2.default.createElement('div', { className: 'tick-container tick-containerR' },  _react2.default.createElement('div', { className: 'ticks ticksR' })),
